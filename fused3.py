...
def control(pi):
    ...
    for w in range(0, walls.shape[0]):  # for each wall
        ...
        if a1[0] == 0:  # vertical wall!!!
        ...
        elif (po[2] == 0 or po[2] == 180) and a1[1] == 0:
            1
        else:
            m = a1[1] / a1[0]
            c = -m * walls[w][1][0] + walls[w][1][1]  # for the wall: y = mx +c
            ...
            if po[2] != 90 and po[2] != 270:
                ...
                crush[0] = pi[0]
            crush[1] = (m * crob + c * np.tan(np.deg2rad(pi[2]))) / (-m + np.tan(np.deg2rad(pi[2])))
...
def main():
    ...
    for i in range(0, 100):
        realpos, u, crash = control(realposold)
